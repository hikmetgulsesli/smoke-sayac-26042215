...
export function SifirlamaOnayDiyalogu({ context }: SifirlamaOnayDiyaloguProps) {
  ...
  return (
    <>
      ...
      <button className="bg-secondary-container text-on-secondary-container rounded-lg p-6 flex-1 flex items-center justify-center">
      <span className="material-symbols-outlined" style={{fontVariationSettings: "'wght' 600"}}>remove</span>
      </button>
      <button className="bg-gradient-to-br from-primary to-primary-container text-on-primary rounded-lg p-6 flex-1 flex items-center justify-center">
      ...
      <div aria-labelledby="modal-title" aria-modal="true" className="fixed inset-0 bg-surface/80 backdrop-blur-md z-40 flex items-center justify-center p-6" role="dialog">
      {/*  Modal Content Container  */}
      <div className="bg-surface-container-highest rounded-xl w-full max-w-md p-8 shadow-[0_0_60px_rgba(0,42,120,0.3)] relative overflow-hidden ring-1 ring-outline-variant/15 flex flex-col gap-6 transform scale-100 opacity-100 transition-all">
      {/*  Icon/Indicator  */}
      <div className="w-12 h-12 rounded-full bg-error-container/20 flex items-center justify-center text-error mx-auto mb-2">
      <span className="material-symbols-outlined text-3xl" style={{fontVariationSettings: "'FILL' 1"}}>warning</span>
      </div>
      {/*  Title & Description  */}
      <div className="text-center flex flex-col gap-2">
      <h2 className="font-headline text-xl font-bold text-on-surface" id="modal-title">Sıfırlama Onayı</h2>
